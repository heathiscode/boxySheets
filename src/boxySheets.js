...
String.prototype.camelCase = function() {
    return this.toString().replace(/-([a-z])/g, function(g) {
...
function BoxySheet() {
    ...
    function layout() {
        ...
        __($$boxy.items).forEach(function(selector) {
            data = $$boxy.items[selector];
            if (data.elements) {
                data.elements.toArray().forEach(function(el) {
                    __(data.properties).forEach(function(property) {
                        ...
                        setProperty(selector, property, val, el);
                    });
                });
            }
        });
    ...
    function setProperty(selector, prop, value, el) {
        ...
        if (
            ...
            return _(el || selector).css(prop, value);
        }
    ...
    function buildBoxy(boxySheet) {
        ...
        __(boxySheet).forEach(function(item) {
            ...
            __(style).forEach(function(property) {
                ...
                /* if it looks like a querySelector, gather our list */
